...
1. **节点颜色属性**：每个节点都有颜色，要么是红色，要么是黑色。
2. **根节点为黑色**：根节点始终是黑色的。
3. **叶节点为黑色**：所有的叶子节点（NIL节点，即空节点）都是黑色的。
4. **红色节点的子节点为黑色**：如果一个节点是红色的，那么它的两个子节点必须是黑色的，即不能有连续的红色节点。
5. **从根到叶的路径上黑色节点数相同**：从任意一个节点到其子孙叶子节点的所有路径上，包含相同数量的黑色节点。

这些特点保证了红黑树的高度平衡，使得其在最坏情况下的时间复杂度为 $O(\log n)$，其中 $n$ 是树中节点的数量。这使得红黑树在插入、删除和查找操作上都具有较好的性能，常用于实现高效的查找和排序数据结构，如Java中的 `TreeMap` 和 `TreeSet` 等。